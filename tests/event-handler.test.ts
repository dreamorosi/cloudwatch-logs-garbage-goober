...
import { type EventDetail, handler } from '../src/event-handler.js';
...
describe('cw-logs-event-handler', () => {
  ...
  it('throws when logGroups is undefined in response', async () => {
    // Prepare
    cwClient.on(DescribeLogGroupsCommand).resolves({});
  ...
  it('creates a deletion schedule for a log group with retention', async () => {
    ...
    // Act
    const result = await handler(event as unknown as EventDetail, context);
  ...
  it('throws when exact match not found even if prefix matches exist', async () => {
